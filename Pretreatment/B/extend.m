...
ini_v = (n-1)/(s*n-1); %图像B(2,1)相当于计算A(1+ini_u,1)，即每步加ini_u
         

Output = zeros(w*m,s*n);            %初始化输出矩阵
for j = 1:s*n;                      %左边两个语句的功能是：z_u,z_v向左取整，u,v取小数，原理如下
